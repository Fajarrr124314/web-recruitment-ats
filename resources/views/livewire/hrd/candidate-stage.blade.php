                        ...
                        <div class="flex h-full flex-col overflow-y-scroll bg-white border-l border-red-100 shadow-2xl relative">
                            ...
                            <div class="px-6 py-5 border-b border-slate-150 flex items-center justify-between bg-white/80 backdrop-blur-md sticky top-0 z-10">
                                <div>
                                    ...
                                    <p class="text-xs text-slate-400 font-bold mt-0.5">Lamaran masuk pada {{ $selectedApplication->created_at->format('d M Y, H:i') }}</p>
                                </div>
                            ...
                            <div class="flex-1 p-6 space-y-6">
                                ...
                                <div class="bg-slate-50/70 p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-4">
                                    <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                                        ...
                                        <div class="flex-1">
                                            <label class="block text-[9px] font-extrabold uppercase tracking-wider text-slate-500 mb-2">Ubah Tahap Rekrutmen</label>
                                                ...
                                                @foreach(['Administrasi', 'Psikotes', 'Interview', 'MCU', 'Hired'] as $targetStage)
                                                    @php
                                                        ...
                                                        $btnTheme = match($targetStage) {
                                                            ...
                                                            'Psikotes' => $isCurrent ? 'bg-red-600 text-white' : 'bg-white text-red-600 border-slate-200 hover:border-red-300',
                                                    ...
                                                    <button wire:click="changeStatus({{ $selectedApplication->id }}, '{{ $targetStage }}')"
                                                        class="px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all border shadow-sm hover:shadow {{ $btnTheme }}">
                                                        {{ $targetStage }}
                                                    </button>
                                                @endforeach
                                            </div>
                                    ...
                                    <div class="border-t border-slate-200 pt-3 flex items-center justify-between">
                                        <span class="text-[9px] text-slate-400 font-extrabold uppercase">Opsi Lainnya:</span>
                                        <button wire:click="openRejectModal" class="px-3 py-1.5 bg-white border border-red-200 text-red-650 hover:bg-red-50 hover:border-red-300 rounded-lg text-xs font-bold transition-colors">
                                            Tolak / Gugur Kandidat
                                        </button>
                                    </div>
                                </div>
                                ...
                                @if($showRejectModal)
                                    <div class="bg-red-50 border border-red-250 p-4 rounded-xl shadow-inner animate-fade-in">
                                        <h3 class="text-red-850 font-bold text-sm mb-2">Konfirmasi Penolakan Kandidat</h3>
                                        <p class="text-xs text-red-650 mb-3">Mohon berikan alasan profesional yang konstruktif. Alasan ini akan ditampilkan kepada kandidat beserta info jeda waktu pendaftaran 1 bulan.</p>
                                        
                                        <!-- Quick Templates -->
                                        <div class="mb-3">
                                            <p class="text-[9px] font-extrabold text-red-700 uppercase tracking-wider mb-2">Pilih Alasan Cepat:</p>
                                            <div class="flex flex-wrap gap-2">
                                                <button type="button" wire:click="$set('rejectReason', 'Mohon maaf, kualifikasi berkas administrasi Anda belum memenuhi kriteria spesifikasi yang kami cari saat ini. Terima kasih!')" 
                                                ...
                                                <button type="button" wire:click="$set('rejectReason', 'Berdasarkan evaluasi hasil psikotes yang kami terima, saat ini profil Anda belum cocok dengan spesifikasi lowongan yang dibuka. Tetap semangat!')" 
                                                    class="px-2.5 py-1 bg-white border border-red-300 text-red-700 hover:bg-red-100 rounded-md text-[10px] font-bold transition-all">
                                                    Gagal Psikotes
                                                </button>
                                        ...
                                        <textarea wire:model="rejectReason" rows="3" class="w-full text-xs p-3 border border-red-200 rounded-lg focus:outline-none bg-white" placeholder="Atau ketik alasan kustom Anda sendiri di sini..."></textarea>
                                        @error('rejectReason')<span class="text-xs text-red-650 font-bold block mt-1">{{ $message }}</span>@enderror

                                        <div class="flex justify-end gap-2 mt-3">
                                            <button wire:click="closeRejectModal" class="px-4 py-1.5 bg-white border border-slate-300 text-slate-650 rounded-lg text-xs font-bold hover:bg-slate-50">Batal</button>
                                            <button wire:click="rejectCandidate" class="px-4 py-1.5 bg-red-600 text-white rounded-lg text-xs font-bold hover:bg-red-700 shadow-md">Tolak Kandidat</button>
                                        </div>
                                    </div>
                                @endif
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div class="bg-white border border-slate-150 p-4 rounded-xl shadow-sm">
                                        <h3 class="text-[9px] font-extrabold uppercase tracking-wider text-slate-400 border-b border-slate-100 pb-1.5 mb-2">Kontak & Posisi</h3>
                                        <p class="text-sm font-bold text-slate-800 leading-snug">{{ $selectedApplication->job_title }}</p>
                                        <div class="mt-2 text-xs text-slate-600 space-y-1.5">
                                            <p><span class="text-slate-400 font-semibold">Email:</span> {{ $selectedApplication->candidate->user->email }}</p>
                                            <p><span class="text-slate-400 font-semibold">WhatsApp:</span> {{ $selectedApplication->candidate->phone }}</p>
                                        </div>
                                    </div>

                                    <div class="bg-white border border-slate-150 p-4 rounded-xl shadow-sm">
                                        <h3 class="text-[9px] font-extrabold uppercase tracking-wider text-slate-400 border-b border-slate-100 pb-1.5 mb-2">Keahlian (Skill)</h3>
                                        <div class="flex flex-wrap gap-1 mt-1">
                                            ...
                                            @empty
                                                <span class="text-xs text-slate-400 italic">Tidak ada keahlian khusus dicantumkan.</span>
                                            @endforelse
                                        </div>
                                    </div>

                                    <div class="sm:col-span-2 bg-white border border-slate-150 p-4 rounded-xl shadow-sm">
                                        <h3 class="text-[9px] font-extrabold uppercase tracking-wider text-slate-400 border-b border-slate-100 pb-1.5 mb-2">Riwayat Pengalaman</h3>
                                ...
                                @if($selectedApplication->answers->count() > 0)
                                    <div class="space-y-3">
                                        <h3 class="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 border-b border-slate-200 pb-1.5">Berkas & Form Dinamis</h3>
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            @foreach($selectedApplication->answers as $ans)
                                                <div class="bg-slate-50 border border-slate-200 p-3 rounded-xl shadow-inner">
                                                    <p class="text-[9px] font-extrabold text-slate-450 mb-1 leading-tight truncate" title="{{ $ans->requirement->question }}">{{ $ans->requirement->question }}</p>
                                                    @if($ans->requirement->type === 'file')
                                                        <a href="{{ Storage::url($ans->answer) }}" target="_blank" 
                                                           class="text-red-600 font-extrabold text-xs hover:underline flex items-center gap-1 bg-red-50 px-2 py-1.5 rounded-lg border border-red-100 w-fit">
                                                    ...
                                                    @else
                                                        <p class="text-xs text-slate-800 break-words font-medium leading-normal">{{ $ans->answer }}</p>
                                                    @endif
                                                </div>
                                            @endforeach
                                        </div>
                                ...
                                <!-- Rubric Collaborative Scoring -->
                                <div class="space-y-4 pt-4 border-t border-slate-200">
                                    <h3 class="text-[10px] font-extrabold uppercase tracking-wider text-slate-500">Penilaian Tahap {{ $stage }} ({{ count($selectedApplication->interviewScores) }})</h3>
                                    
                                    @if(session()->has('rating_success'))
                                        <div class="p-3 rounded-lg bg-green-50 border border-green-200 text-green-750 text-xs font-semibold flex items-center space-x-1.5 animate-fade-in">
                                    ...
                                    <div class="space-y-3">
                                        @forelse($selectedApplication->interviewScores as $score)
                                            <div class="bg-white border border-slate-150 p-4 rounded-xl shadow-sm space-y-2">
                                                <div class="flex justify-between items-center border-b border-slate-100 pb-2">
                                                    <div class="flex items-center gap-2">
                                                        <div class="h-7 w-7 rounded-full bg-slate-100 text-slate-700 flex items-center justify-center font-bold text-xs">
                                                            {{ substr($score->interviewer->name, 0, 1) }}
                                                        </div>
                                                        <span class="text-xs font-bold text-slate-800">{{ $score->interviewer->name }}</span>
                                                    </div>
                                                    <div class="flex items-center gap-1.5 text-amber-500">
                                                        <span class="text-xs font-extrabold text-slate-800">★ {{ number_format($score->rating, 1) }}</span>
                                                    </div>
                                                </div>
                                                
                                                <!-- Rubric Breakdown -->
                                                <div class="grid grid-cols-2 gap-2 text-[10px]">
                                                    <div><span class="text-slate-400 font-semibold">{{ $dimensions[0]['label'] }}:</span> <span class="font-extrabold">{{ $score->technical_rating }}/5</span></div>
                                                    <div><span class="text-slate-400 font-semibold">{{ $dimensions[1]['label'] }}:</span> <span class="font-extrabold">{{ $score->communication_rating }}/5</span></div>
                                                    <div><span class="text-slate-400 font-semibold">{{ $dimensions[2]['label'] }}:</span> <span class="font-extrabold">{{ $score->problem_solving_rating }}/5</span></div>
                                                    <div><span class="text-slate-400 font-semibold">{{ $dimensions[3]['label'] }}:</span> <span class="font-extrabold">{{ $score->culture_fit_rating }}/5</span></div>
                                                </div>

                                                @if($score->notes)
                                                    <p class="text-xs text-slate-600 italic bg-slate-50 p-2.5 rounded-lg border border-slate-100">"{{ $score->notes }}"</p>
                                                @endif
                                                <span class="text-[9px] text-slate-400 font-semibold block">{{ $score->created_at->diffForHumans() }}</span>
                                            </div>
                                        @empty
                                            <p class="text-xs text-slate-400 italic font-medium">Belum ada penilaian terdaftar.</p>
                                        @endforelse
                                    </div>

                                    <!-- Assessment Entry Form -->
                                    <form wire:submit.prevent="submitAssessment" class="bg-red-50/20 p-4 border border-red-100 rounded-2xl space-y-4">
                                        <span class="text-[9px] font-black text-red-500 uppercase tracking-widest block mb-2">Form Penilaian Objektif</span>
                                        ...
                                        <div class="space-y-3.5">
                                            @foreach($dimensions as $index => $dim)
                                                ...
                                                @endphp
                                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-white p-3 rounded-xl border border-slate-100">
                                                    <div>
                                                        <span class="text-xs font-bold text-slate-800 block">{{ $dim['label'] }}</span>
                                                        <span class="text-[10px] text-slate-400 font-medium leading-tight block">{{ $dim['desc'] }}</span>
                                                    </div>
                                                    <!-- Rating Buttons -->
                                                    <div class="flex items-center gap-1 shrink-0 mt-2 sm:mt-0">
                                                        @for($val = 1; $val <= 5; $val++)
                                                            <button type="button" wire:click="$set('{{ $prop }}', {{ $val }})"
                                                                class="w-6 h-6 rounded-md text-[10px] font-bold flex items-center justify-center border transition-all
                                                                {{ $this->{$prop} === $val
                                        ...
                                        <!-- Notes -->
                                        <div>
                                            <label class="block text-[9px] font-extrabold uppercase text-slate-500 tracking-wider mb-1.5">Catatan Evaluasi / Rekomendasi</label>
                                            <textarea wire:model="ratingNotes" rows="2" class="w-full text-xs p-3 border border-slate-200 rounded-xl focus:outline-none bg-white" placeholder="Masukkan ringkasan interview teknis, kekuatan, kelemahan, atau keputusan..."></textarea>
                                        </div>

                                        <button type="submit" class="w-full py-2.5 rounded-xl text-xs font-extrabold tracking-wider uppercase text-white bg-gradient-to-r {{ $theme['gradient'] }} hover:brightness-105 transition-all shadow-md">
                                            Simpan Penilaian
                                        </button>
                                    </form>
                                </div>
