            ...
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center">
                    <div class="flex-shrink-0 flex items-center space-x-2">
                        <div class="h-10 w-10 rounded-xl bg-gradient-to-tr from-red-500 to-rose-600 flex items-center justify-center shadow-lg shadow-red-500/25">
                            <svg class="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                            </svg>
                        </div>
                        <span class="text-xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-red-400 to-rose-400">LogoPerusahaan</span>
                    </div>
                </div>
                ...
                <div class="hidden md:flex md:items-center md:space-x-4">
                    @auth
                        ...
                        <span class="text-sm text-slate-500">
                            {{ auth()->user()->name }} 
                            <span class="ml-1.5 px-2 py-0.5 rounded-full text-xs font-semibold {{ auth()->user()->isHrd() ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20' }}">
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="inline">
                            @csrf
                            <button type="submit" class="ml-2 px-3 py-2 rounded-lg text-sm font-medium text-rose-400 hover:bg-rose-500/10 transition-colors flex items-center space-x-1">
                    ...
                    @else
                        ...
                        <a href="{{ route('register') }}" class="ml-2 bg-gradient-to-r from-red-500 to-rose-600 hover:from-red-600 hover:to-rose-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-md shadow-red-500/15 hover:shadow-red-500/25 transition-all">Daftar</a>
                    @endauth
                </div>
            ...
            <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-white/95 backdrop-blur-md border-b border-red-100">
                @auth
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="w-full text-left block px-3 py-2 rounded-lg text-base font-medium text-rose-400 hover:bg-rose-500/10 transition-colors">
                            Keluar
                        </button>
                    </form>
                @else
                    ...
                    <a href="{{ route('register') }}" class="block px-3 py-2 rounded-lg text-base font-medium bg-gradient-to-r from-red-500 to-rose-600 text-white">Daftar</a>
                @endauth
            </div>
        </div>
    </nav>
