...
<body class="bg-red-50/50 font-sans text-slate-900 antialiased h-screen flex flex-col md:flex-row overflow-hidden w-full" x-data="{ sidebarOpen: false, desktopSidebarOpen: true }">
    ...
    <!-- Sidebar -->
    <aside :class="{ 'translate-x-0': sidebarOpen, '-translate-x-full': !sidebarOpen, 'md:w-64': desktopSidebarOpen, 'md:w-20': !desktopSidebarOpen }" class="fixed md:static inset-y-0 left-0 z-50 w-64 md:translate-x-0 bg-white/95 md:bg-white/80 backdrop-blur-xl border-r border-red-100 flex flex-col shadow-2xl md:shadow-lg shadow-red-500/5 transition-all duration-150 h-full overflow-hidden shrink-0">
        <div class="h-16 flex items-center border-b border-red-100 bg-white/40 backdrop-blur-md shrink-0 transition-all w-full" :class="desktopSidebarOpen ? 'px-4 justify-between' : 'px-4 md:px-0 md:justify-center justify-between'">
            ...
            <button @click="desktopSidebarOpen = !desktopSidebarOpen" class="hidden md:block text-slate-500 hover:text-red-600 hover:bg-red-50 p-1.5 rounded-lg focus:outline-none shrink-0 transition-colors" title="Toggle Sidebar">
                <svg class="w-5 h-5 transition-transform duration-150" :class="desktopSidebarOpen ? '' : 'rotate-180'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <button @click="if(!pinned) open = !open" title="Proses Kandidat"
                   ...
                   :class="desktopSidebarOpen ? 'px-4' : 'px-4 md:px-0 md:justify-center'">
                    <div class="flex items-center gap-3">
                        <svg class="w-5 h-5 shrink-0 transition-transform duration-150 group-hover:scale-110 group-hover:rotate-3 {{ (request()->routeIs('hrd.dashboard') || request()->routeIs('hrd.process')) ? 'text-red-500' : 'text-slate-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="border-t border-red-100 shrink-0 transition-all" :class="desktopSidebarOpen ? 'p-3' : 'p-3 md:p-2'">
            ...
            <div class="bg-gradient-to-r from-red-50/90 to-red-100/40 backdrop-blur-md border border-red-100 p-2.5 rounded-2xl mb-2.5 shadow-inner hover:shadow transition-all duration-150 relative group"
            ...
            <a href="{{ route('hrd.activity-logs') }}" wire:navigate title="Log Aktivitas"
               ...
               :class="desktopSidebarOpen ? 'px-4 gap-2' : 'px-0'">
                <svg class="w-4 h-4 shrink-0 transition-transform duration-150 group-hover:scale-110 {{ request()->routeIs('hrd.activity-logs') ? 'text-red-500 animate-pulse' : 'text-slate-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <form action="{{ route('logout') }}" method="POST">
                ...
                <button type="submit" title="Keluar" 
                        class="group w-full flex items-center justify-center py-2 border border-red-200 rounded-xl text-red-600 bg-white hover:bg-red-50 transition-all duration-150 hover:scale-[1.02] hover:shadow-sm font-semibold text-xs"
                        :class="desktopSidebarOpen ? 'px-4 gap-2' : 'px-0'">
                    <svg class="w-4 h-4 shrink-0 transition-transform duration-150 group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
